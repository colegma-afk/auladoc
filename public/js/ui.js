...
// ---------- Iconos (SVG stroke 24) ----------
UI.ICONS = {
  logo: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10 12 5 2 10l10 5 10-5z"/><path d="M6 12v5c0 1.7 2.7 3 6 3s6-1.3 6-3v-5"/><path d="M22 10v6"/></svg>',
  ...
  play: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="m10 8 6 4-6 4V8z"/></svg>',
  ...
  cap: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10 12 5 2 10l10 5 10-5z"/><path d="M6 12v5c0 1.7 2.7 3 6 3s6-1.3 6-3v-5"/><path d="M22 10v6"/></svg>',
...
UI.toast = (msg, type = 'success') => {
  const icons = { success: UI.ICONS.checkCircle, error: UI.ICONS.alert, info: UI.ICONS.info };
...
UI.openModal = (title, bodyHtml, footHtml, opts = {}) => {
  ...
  overlay.innerHTML = `
    <div class="modal ${opts.lg ? 'modal-lg' : ''}">
      <div class="modal-head">
        <h3>${title}</h3>
        <button class="modal-x" data-close><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg></button>
      </div>
  ...
  document.addEventListener('keydown', UI._escHandler = e => { if (e.key === 'Escape') UI.closeModal(); });
  return overlay;
};
UI.closeModal = () => {
  const overlay = document.getElementById('modalOverlay');
...
// ---------- Avatar ----------
UI.avatar = (name, cls = '') =>
  `<span class="avatar ${cls}" style="background:${Domain.avatarColor(name)}">${Domain.initials(name)}</span>`;

// ---------- Progreso ----------
UI.progressBar = (pct, cls = '') =>
  `<div class="progress ${cls}"><i style="width:${Math.min(100, Math.max(0, pct))}%"></i></div>`;

// ---------- Tarjeta de curso (reutilizable) ----------
UI.courseCard = (c) => {
  const lessons = c.modules ? c.modules.reduce((n, m) => n + m.lessons.length, 0) : c.totalLessons || 0;
  const btn = c.enrolled
    ? (c.progress.pct >= 100
      ? `<a class="btn btn-ghost btn-sm" href="#/curso/${c.id}">${UI.ICONS.checkCircle} Ver curso</a>`
      : `<a class="btn btn-primary btn-sm" href="#/curso/${c.id}">${UI.ICONS.play} Continuar · ${c.progress.pct}%</a>`)
    : `<a class="btn btn-ghost btn-sm" href="#/curso/${c.id}">Ver detalles</a>`;
  return `
  <div class="card course-card card-hover">
    <a class="cover" href="#/curso/${c.id}">
      <img src="${c.cover}" alt="${c.title}" loading="lazy">
      <span class="chip">${c.category}</span>
      <span class="dur">${UI.ICONS.clock} ${c.hours} h</span>
    </a>
    <div class="cc-body">
      <a href="#/curso/${c.id}" style="color:inherit"><h3 class="cc-title">${c.title}</h3></a>
      <div class="cc-meta">
        <span>${UI.ICONS.video} ${lessons} ${Domain.plural(lessons, 'lección', 'lecciones')}</span>
        <span>${UI.ICONS.grad} ${c.level}</span>
        <span>${UI.ICONS.users} ${c.enrolledCount} inscritos</span>
      </div>
      ${c.enrolled ? `<div>${UI.progressBar(c.progress.pct)}<div style="font-size:12px;color:var(--muted);margin-top:5px">${c.progress.pct}% completado</div></div>` : ''}
      <div class="cc-foot">${btn}</div>
    </div>
  </div>`;
};

// ---------- Estado vacío ----------
UI.empty = (icon, title, sub) => `
  <div class="empty-state">
    ${UI.ICONS[icon] || UI.ICONS.book}
    <b>${title}</b>
    <span>${sub || ''}</span>
  </div>`;
